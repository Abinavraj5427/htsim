...
class MtpTrafficLogger : public TrafficLogger {
   public:
      void logTraffic(Packet& pkt, Logged& location, TrafficEvent ev);
      static string event_to_str(RawLogEvent& event);
};
...
class MsgQueueLoggerSimple : public MsgQueueLogger {
 public:
    void logQueue(AbstractQueue& queue, MsgQueueEvent ev, Packet& pkt);
    static string event_to_str(RawLogEvent& event);
};

// sampling means periodic queue loggers
...
class SinkLoggerSampling : public Logger, public EventSource {
 public:
    ...
    virtual void monitorSink(DataReceiver* sink);
...
class MtpSinkLoggerSampling : public SinkLoggerSampling {
   virtual void doNextEvent();
   void monitorSink(DataReceiver* sink);
   public:
      MtpSinkLoggerSampling(simtime_picosec period, EventList& eventlist);
      static string event_to_str(RawLogEvent& event);
};
