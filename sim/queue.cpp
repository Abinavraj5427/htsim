...
AbstractQueue::AbstractQueue(linkspeed_bps bitrate, mem_b maxsize, EventList& eventlist, AbstractQueueLogger* logger)
: EventSource(eventlist, "queue"), 
_maxsize(maxsize), _bitrate(bitrate), _logger(logger){
    _ps_per_byte = (simtime_picosec)((pow(10.0, 12.0) * 8) / _bitrate);
    stringstream ss;
    ss << "abstractqueue(" << bitrate/1000000 << "Mb/s," << maxsize << "bytes)";
    _nodename = ss.str();
}

Queue::Queue(linkspeed_bps bitrate, mem_b maxsize, EventList& eventlist, 
	     QueueLogger* logger)
  : AbstractQueue(bitrate, maxsize, eventlist, logger), _num_drops(0)
{
    _queuesize = 0;
    stringstream ss;
...
Queue::completeService()
{
    ...
    if (_logger) ((QueueLogger*) _logger)->logQueue(*this, QueueLogger::PKT_SERVICE, *pkt);
...
{   
    if(pkt.type() == MTP){
        ...
        if(_queuesize + pkt.size() > 0.5 * _maxsize){
            mtppkt.set_ecn();
        }
    }
	...
	/* if the packet doesn't fit in the queue, drop it */
        if (_logger) 
            ((QueueLogger*) _logger)->logQueue(*this, QueueLogger::PKT_DROP, pkt);
    ...
    _queuesize += pkt.size();
    if (_logger) ((QueueLogger*) _logger)->logQueue(*this, QueueLogger::PKT_ENQUEUE, pkt);
...
/**
 * Approximates the time until packet is released
 * from the queue.
*/
simtime_picosec
