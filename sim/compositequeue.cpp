...
//   cout << "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB" << endl;
...
CompositeQueue::receivePacket(Packet& pkt)
{
    ...
    if (!pkt.header_only()){
		if (_queuesize_low+pkt.size() <= _maxsize  || drand()<0.5) {
			...
			if (_queuesize_low+pkt.size()>_maxsize){
				...
				if (_logger) ((QueueLogger*) _logger)->logQueue(*this, QueueLogger::PKT_TRIM, pkt);
				
				if (_queuesize_high+booted_pkt->size() > _maxsize){
					if (booted_pkt->reverse_route()  && booted_pkt->bounced() == false) {
						//return the packet to the sender
						if (_logger) ((QueueLogger*) _logger)->logQueue(*this, QueueLogger::PKT_BOUNCE, *booted_pkt);
						...
						booted_pkt->free();
						if (_logger) ((QueueLogger*) _logger)->logQueue(*this, QueueLogger::PKT_DROP, pkt);
					}
				}  
				else {
	...
	if (_queuesize_high+pkt.size() > _maxsize){
		...
		if (pkt.reverse_route()  && pkt.bounced() == false) {
			//return the packet to the sender
			if (_logger) ((QueueLogger*) _logger)->logQueue(*this, QueueLogger::PKT_BOUNCE, pkt);
