...
class Logger {
 ...
 public:
    enum EventType { QUEUE_EVENT=0, TCP_EVENT=1, TCP_STATE=2, TRAFFIC_EVENT=3, 
		     ...
		     NDP_SINK = 18, NDP_MEMORY = 19, MSG_QUEUE_EVENT=20, MTP_SINK=21};
...
class AbstractQueueLogger : public Logger {
};

class QueueLogger : public AbstractQueueLogger  {
 public:
...
class MsgQueueLogger: public AbstractQueueLogger {
   public:
      enum MsgQueueEvent { MSG_ENQUEUE=1, MSG_PROCESSING=2, MSG_SERVICE=3, MSG_DROP=4, MSG_EXPIRE=5 };
      virtual void logQueue(AbstractQueue& queue, MsgQueueEvent ev, Packet& pkt) = 0;
      virtual ~MsgQueueLogger(){};
};
...
class NdpLogger  : public Logger {
 public:
    ...
    // used for type of event when logging
...
/* Logs MTP related events */
class MtpLogger : public Logger{
   public:
      enum MtpEvent {};
      enum MtpSinkRecord { RATE = 0 };
      virtual void logMtp(MtpSrc &src, MtpEvent ev) = 0;
      virtual ~MtpLogger(){};
};
