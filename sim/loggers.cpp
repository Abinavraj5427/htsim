...
void MsgQueueLoggerSimple::logQueue(AbstractQueue& queue, MsgQueueEvent ev, Packet& pkt){
	MsgQueue& msgqueue = dynamic_cast<MsgQueue&>(queue);
	MtpPacket& mtppkt = dynamic_cast<MtpPacket&>(pkt);
	switch(ev){
		case MSG_ENQUEUE:{
			_logfile->writeRecord(
				Logger::MSG_QUEUE_EVENT, msgqueue.id, ev, mtppkt.get_src(), mtppkt.get_message_id(), msgqueue.queuesize());
			break;
		}
		case MSG_PROCESSING:{
			_logfile->writeRecord(
				Logger::MSG_QUEUE_EVENT, msgqueue.id, ev, mtppkt.get_src(), mtppkt.get_message_id(), mtppkt.get_ttl_pkts());
			break;
		}
		case MSG_SERVICE:{
			_logfile->writeRecord(
		...
		case MSG_DROP:{
			_logfile->writeRecord(
				Logger::MSG_QUEUE_EVENT, msgqueue.id, ev, mtppkt.get_src(), mtppkt.get_message_id(), msgqueue.total_drops());
			break;
		}
		case MSG_EXPIRE:{
			uint32_t combo_id = ((mtppkt.get_src()<<16) + mtppkt.get_message_id());
			_logfile->writeRecord(
				Logger::MSG_QUEUE_EVENT, msgqueue.id, ev, (double) combo_id, msgqueue.queuesize(), msgqueue.total_expirations());
			break;
		}
		default:{
			cout << "Unrecognized MsgQueue event: " << ev << endl;
			break;
		}
	}
}

string MsgQueueLoggerSimple::event_to_str(RawLogEvent& event){
	stringstream ss;
    ss << fixed << setprecision(9) << event._time;
	assert(event._type == Logger::QUEUE_EVENT);
	ss << " ID " << event._id;

	if(event._ev == MsgQueueLogger::MSG_EXPIRE){
		uint32_t combo_id = (uint32_t) event._val1;
		uint32_t src = combo_id >> 16;
		uint32_t msg_id = combo_id - (src << 16);
		ss << src << " " << msg_id;
	}else{
		ss << event._val1 << " " << event._val2;
	}

	switch(event._ev){
		case MsgQueueLogger::MSG_ENQUEUE:
			ss << " Ev MSG_ENQUEUE " << event._val3;
			break;
		case MsgQueueLogger::MSG_PROCESSING:
			ss << " Ev MSG_PROCESSING " << event._val3;
			break;
		case MsgQueueLogger::MSG_SERVICE:
			ss << " Ev MSG_SERVICE " << event._val3;
			break;
		case MsgQueueLogger::MSG_DROP:
			ss << " Ev MSG_DROP " << event._val3;
			break;
		case MsgQueueLogger::MSG_EXPIRE:
			ss << " Ev MSG_EXPIRE " << event._val2 << " " << event._val3;
			break;
		default:
			break;
	}
...
/**MTP SINK LOGGER*/
// Goal: keep track of all received messages
void MtpSinkLoggerSampling::monitorSink(DataReceiver* sink){
	_sinks.push_back(sink);
}

// write record of total messages rcvd every period
void MtpSinkLoggerSampling::doNextEvent(){
	for(DataReceiver* sink: _sinks){
		_logfile->writeRecord(_sink_type, sink->get_id(), _event_type, 
		((MtpSink*) sink)->get_total_received(), 0, 0);
	}
}

// logger initialized msg
MtpSinkLoggerSampling::MtpSinkLoggerSampling(simtime_picosec period, EventList& eventlist):
    SinkLoggerSampling(period, eventlist, Logger::MTP_SINK, MtpLogger::RATE)
{
    cout << "MtpSinkLoggerSampling(p=" << timeAsSec(period) << " init \n";
}

// translate the event, when reading the log file
string MtpSinkLoggerSampling::event_to_str(RawLogEvent& event){
	stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    switch(event._type) {
		case Logger::MTP_SINK:
			assert(event._ev == MtpLogger::RATE);
			ss << " Type MTP_SINK ID " << event._id << " Ev RATE"
			<< " TotalRcvd: " << (uint64_t)event._val1;
			break;
		default:
			ss << "Unknown event " << event._type;
			break;
